...
local function preProcessHeaders(fileContents)
	...
	for line in fileContents:gmatch("[^\r\n]+") do
		-- drop # lines, // comments and lines that contain extern "C"
		if not (line:find("^#") or line:find("^%s*//") or line:find('extern "C"')) then
...
local cdefs = preProcessHeaders(C_FileSystem.ReadFile("deps/webgpu-native/webgpu.h"))
local prefix = "return [[\n"
local suffix = "\n]]"
C_FileSystem.WriteFile("cdefs.lua", prefix .. cdefs .. suffix)
...
ffi.cdef(glfw_cdefs)
-- TODO separate bindings, probably part of runtime even?
